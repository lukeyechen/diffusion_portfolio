...
LOOKBACK = 50         # app's preferred 3M lookback for 5 assets
GAMMA = 3.0
M = 500
BETA = 1.0
N_STEPS = 100
...
def perf_metrics(r, periods_per_year=4):
...
def main():
    returns = download_yahoo_returns(
        ...
        interval="1mo",
        return_horizon_months=3,
    )
    returns = returns[TICKERS].dropna().copy()

    if len(returns) <= LOOKBACK:
        raise RuntimeError(f"Need more than {LOOKBACK} 3M returns, got {len(returns)}")
    ...
    print("\n=== 3-MONTH OUT-OF-SAMPLE BACKTEST ===")
    print(f"Tickers: {', '.join(TICKERS)}")
    print(f"Source history requested: {START} to {END}")
    print(f"Common 3M return observations: {len(returns)}")
    print(f"Rolling lookback: {LOOKBACK} non-overlapping 3M observations (~12.5 years)")
    print(f"OOS quarters: {len(detail)}")
    ...
    display_cols = ["Total return", "CAGR", "Annualized vol", "Annualized Sharpe (rf=0)", "Max drawdown", "Positive quarters", "Final $10,000"]
    out = gross_summary[display_cols].copy()
    print(out.to_string(float_format=lambda x: f"{x:.6f}"))

    diff_beats = float((detail["diff_minus_ew"] > 0).mean())
    avg_excess_q = float(detail["diff_minus_ew"].mean())
    ...
    print("\n=== DIRECT COMPARISON ===")
    print(f"Diffusion beats equal-weight quarters: {diff_beats:.2%}")
    print(f"Average diffusion minus equal-weight return per 3M period: {avg_excess_q:.4%}")
    print(f"Cumulative wealth advantage (return points): {(cumulative_diff - cumulative_ew):.4%}")
    print(f"Average diffusion turnover per quarter: {detail['turnover_diffusion'].mean():.4f}")
    print(f"Average equal-weight turnover per quarter: {detail['turnover_equal_weight'].mean():.4f}")
    ...
    # Current recommendation from the latest 50 complete 3M observations.
    ...
    print("\n=== LATEST 3M DIFFUSION WEIGHTS FROM THE SAME RULE ===")
    print(f"T={T_latest:.6f}, b*/n={b_latest/LOOKBACK:.6f}, s_T^2={signal_latest:.6f}")
    for ticker, w in zip(TICKERS, w_latest):
        print(f"{ticker}: {w:.4%}")

    detail.to_csv("three_month_backtest_detail.csv", index=False)
    gross_summary.to_csv("three_month_backtest_summary.csv")
