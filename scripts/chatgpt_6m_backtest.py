...
LOOKBACK = 20         # 20 non-overlapping 6M observations = ~10 years
GAMMA = 3.0
M = 500
BETA = 1.0
N_STEPS = 100
...
def perf_metrics(r, periods_per_year=2):
...
def main():
    returns = download_yahoo_returns(
        ...
        interval="1mo",
        return_horizon_months=6,
    )
    returns = returns[TICKERS].dropna().copy()

    if len(returns) <= LOOKBACK:
        raise RuntimeError(f"Need more than {LOOKBACK} 6M returns, got {len(returns)}")
    ...
    print(f"Common 6M return observations: {len(returns)}")
    print(f"Rolling lookback: {LOOKBACK} non-overlapping 6M observations (~10 years)")
    print(f"OOS half-years: {len(detail)}")
    ...
    display_cols = ["Total return", "CAGR", "Annualized vol", "Annualized Sharpe (rf=0)", "Max drawdown", "Positive half-years", "Final $10,000"]
    ...
    print("\n=== DIRECT COMPARISON ===")
    print(f"Diffusion beats equal-weight half-years: {diff_beats:.2%}")
    print(f"Average diffusion minus equal-weight return per 6M period: {avg_excess:.4%}")
    print(f"Cumulative wealth advantage (return points): {(cumulative_diff - cumulative_ew):.4%}")
    print(f"Average diffusion turnover per half-year: {detail['turnover_diffusion'].mean():.4f}")
    print(f"Average equal-weight turnover per half-year: {detail['turnover_equal_weight'].mean():.4f}")
    ...
    print("\n=== LATEST 6M DIFFUSION WEIGHTS FROM THE SAME RULE ===")
    print(f"T={T_latest:.6f}, b*/n={b_latest/LOOKBACK:.6f}, s_T^2={signal_latest:.6f}")
    for ticker, w in zip(TICKERS, w_latest):
        print(f"{ticker}: {w:.4%}")

    detail.to_csv("six_month_backtest_detail.csv", index=False)
    gross_summary.to_csv("six_month_backtest_summary.csv")
